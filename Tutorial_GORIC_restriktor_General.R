...
out <- goric(fit.lm, H1, H2) # or: out <- goric(fit.lm, H1, H2, type = "goric")
summary(out) 
# gorica
set.seed(123) # Set seed value
...
# goric (default)
set.seed(123) # Set seed value
out_s <- goric(fit.lm_s, H1, comparison = "complement")
summary(out_s) 
...
# goric (default)
set.seed(123) # Set seed value
out <- goric(fit.lm, H1) # or: out <- goric(fit.lm, H1, H2, type = "goric")
summary(out) 
...
H2 <- "x1 > 0; x2 > 0"
set.seed(123) # Set seed value
out <- goric(fit.lm, H1, H2, comparison = "none")
summary(out)
...
H1 <- "x1 > 0"
set.seed(123) # Set seed value
out <- goric(fit.lm, H1, comparison = "complement")
out
summary(out)
...
H2 <- "x1 > 0; x2 > 0"
set.seed(123) # Set seed value
out <- goric(fit.lm, H2, comparison = "complement")
summary(out)
